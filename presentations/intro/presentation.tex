...
\begin{frame}
  \frametitle{Types of Recurrent Models}
  \begin{figure}
    \includegraphics[height=3cm, valign=c]{assets/typesmodels}
    \caption{\footnotesize Source: \href{http://cs231n.stanford.edu/slides/2017/cs231n_2017_lecture10.pdf}{Fei-Fei Li \& Justin Johnson \& Serena Yeung, Standford CS231, Recurrent Neural Networks}}
  \end{figure}
  {\footnotesize
  \begin{center}
    \begin{tabular}{lccc}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Union-Find Disjoint Set (UFDS) Algorithm} to group columns by \textbf{Intersection Over Union (IOU)}
    \begin{figure}
      \centering
      \begin{tikzpicture}[shorten >=1pt,node distance=1.1cm,on grid,auto, every node/.style={scale=0.75},
        el/.style={inner sep=2pt, align=left, sloped}]

        \node[circle, draw] (r0) {$0$};
        \node[circle, draw] (r1) [right=of r0] {$1$};
        \node[circle, draw] (r2) [right=of r1] {$2$};
        \node[circle, draw] (r3) [below=of r0] {$3$};
        \node[circle, draw] (r4) [right=of r3] {$4$};
        \node[circle, draw] (r5) [right=of r4] {$5$};
        \node[circle, draw] (r6) [below=of r3] {$6$};
        \node[circle, draw] (r7) [right=of r6] {$7$};
        \node[circle, draw] (r8) [right=of r7] {$8$};
      ...
      \begin{tikzpicture}[shorten >=1pt,node distance=1.1cm,on grid,auto, every node/.style={scale=0.75},
        ...
        \path[->]
          (r3) edge [bend right] node[] {} (r0)
          (r6) edge [bend left] node[] {} (r0)

          (r4) edge [bend right] node[] {} (r1)
          (r7) edge [bend left] node[] {} (r1)

          (r5) edge [bend right] node[] {} (r2)
          (r8) edge [bend left] node[] {} (r2)
...
\begin{frame}
  \frametitle{Automatic Evaluation: BLEU}
  \begin{itemize}
    \item \textcolor{ProcessBlue}{\underline{B}i\underline{l}ingual \underline{E}valuation \underline{U}nderstudy}
    \item For an candidate and reference translation:
    \begin{itemize}
      \item Compare \textit{n}-gram precision of sizes 1 to 4
      \item Brevity penalty for short translations
    \end{itemize}
  \end{itemize}
  {\footnotesize
  \begin{center}
    \begin{tabular} {rll}
      \texttt{Source}    & \foreignlanguage{russian}{У меня есть навыки управления временем моркови} & {} \\
      \texttt{Reference} & I have the time management skills of a carrot & 1.00 \\
      \midrule
      \texttt{A}         & \textcolor{Red}{You got} \textcolor{RedViolet}{the} \textcolor{Red}{scheduling} \textcolor{Turquoise}{skills of a} \textcolor{Red}{banana} & 0.28 \\
      \texttt{B}         & \textcolor{ForestGreen}{I have the time} \textcolor{Red}{to plant lettuce} & \textbf{0.39} \\
      \texttt{C}         & \textcolor{NavyBlue}{I have} \textcolor{Turquoise}{time management skills} \textcolor{Red}{for} \textcolor{NavyBlue}{a carrot} & 0.35 \\
    \end{tabular}
  \end{center}
  }
  \begin{itemize}
    \item \textbf{Key: \textcolor{ForestGreen}{4-gram}, \textcolor{Turquoise}{Trigram}, \textcolor{NavyBlue}{Bigram}, \textcolor{RedViolet}{Unigram}, \textcolor{Red}{No Match}}
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{Outline Review}
  \begin{itemize}
    \item We have covered:
    \begin{itemize}
      \item Seq2Seq Models, Encoders, Decoders
      \item Handling Vocabulary and Word Embeddings
      \item Training Tips and Techniques
    \end{itemize}
    \item Part 2 mixes concepts and practical applications:
    \begin{itemize}
      \item Table Pipeline Review
      \item Decoding with Beam Search
      \item Modeling Recurrent Relations (RNNs)
      \item Updating the Column Segmentation Model
      \item Other applications of Sequence Modeling
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{Quick Review: Table Extraction}
  % update with prettier one from test
  \begin{figure}
    \includegraphics[height=7.5cm, valign=c]{assets/pipeline_img}
  \end{figure}
\end{frame}
...
\begin{frame}
  ...
  % update with prettier one from test
  \begin{figure}
    \includegraphics[height=7.5cm, valign=c]{assets/pipeline_mask}
  \end{figure}
\end{frame}
...
\begin{frame}
  ...
  % update with prettier one from test
  \begin{figure}
    \includegraphics[height=6cm, valign=c]{assets/pipeline_tesseract}
  \end{figure}
\end{frame}
...
\begin{frame}
  ...
  % update with prettier one from test
  \begin{figure}
    \includegraphics[height=7.5cm, valign=c]{assets/pipeline_csv}
  \end{figure}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item How to determine what's optimal? \textbf{N-Gram Lanugage Model}
  \end{itemize}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Modeling Recurrent Relations with RNNs}

\begin{frame}
  \frametitle{RNN Forward Propagation}
  \begin{itemize}
    \item \textbf{Concept:} Allow current sample to mix with previous information
    \item Learn from the pattern's history
  \end{itemize}
  \begin{figure}
    \centering
    \includegraphics[width=0.75\textwidth, valign=c]{assets/rnnt1}
  \end{figure}
\end{frame}
...
\begin{frame}
  \frametitle{RNN Forward Propagation}
  \begin{itemize}
    \item We can unfold the computation graph
  \end{itemize}
  \begin{figure}
    \centering
    \includegraphics[width=0.75\textwidth, valign=c]{assets/rnnt2}
  \end{figure}
\end{frame}
...
\begin{frame}
  \frametitle{RNN Forward Propagation}
  \begin{itemize}
    \item Notice how each cell output is dependent on all prior computations
  \end{itemize}
  \begin{figure}
    \centering
    \includegraphics[width=0.75\textwidth, valign=c]{assets/rnnt3}
  \end{figure}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Hence gradients flow through the entire unfolded graph
    \item Note: we reuse the cell, so the parameters are the same at each time step
  \end{itemize}
  \begin{figure}
    \centering
    \includegraphics[width=0.75\textwidth, valign=c]{assets/bptt}
  \end{figure}
\end{frame}
...
\frametitle{Aside: The Influence of Padding in RNNs}
  \begin{itemize}
    \item Assume the embedding $E\left[ \textcolor{WildStrawberry}{\langle \texttt{PAD} \rangle} \right] = \mathbf{\vv{0}}$
    ...
    \begin{itemize}
      ...
      \item So be carful if taking the last output of a sequence with padding
      \item Make sure that the reverse direction in bidirectionals are not computing padded states
    \end{itemize}
    \item \textbf{Question:} Why does this matter?
    \pause
    \item \textbf{Answer:} RNNs assume $\mathbf{\vv{0}}$ initial hidden state
    \pause
    \item \textbf{Question:} Does this mean we learn the amount of padding for a given sequence?
    \pause
    \item \textbf{Answer:} Most likely, since gradients now incorporate repeated application of activation functions for the bias term.
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{Column Segmentation: Merge the Encoder and Decoder}
  \begin{itemize}
    \item Just use the encoder's embeddings and recurrent layers, with an decoding output layer
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item Jointly train over an entire table
    \end{itemize}
  \end{itemize}
  \begin{figure}
    \centering
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item AKA the reverse direction has the immediate context of the end of the row
      \item And vice-versa
    \end{itemize}
  \end{itemize}
  \begin{figure}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item \textbf{Note:}
    \begin{itemize}
      \item We use the concept of \textit{deep transition} in Seq2Seq for joint training between rows
      \item Our training exploits the finding that we can capture enough signal in the hidden state encoding
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{Possible Experiments}
  \begin{itemize}
    \item Features
    \begin{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item \href{https://www.aclweb.org/anthology/P02-1040.pdf}{Papineni et al., BLEU: a Method for Automatic Evaluation of Machine Translation}
