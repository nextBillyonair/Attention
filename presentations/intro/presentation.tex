...
\section{Bill's Application: Column Segmention}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item Numeric Value $\rightarrow \langle \texttt{NUM} \rangle$
    \end{itemize}
...
\begin{frame}
  \frametitle{Regular Seq2Seq}
  \begin{itemize}
    \item Translation Task
    \item Given a sequence of tokens
    \item Translate into the same set of tokens, but with a special $\langle \texttt{SEG} \rangle$ token
  \end{itemize}
  \vspace{5mm}
  \begin{equation*}
    \centering
...
\begin{frame}
  \frametitle{Secondary Approach: Throw away the decoder}
  \begin{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Features
    \begin{itemize}
      \item Use segment location information
      \item Tesseract gives bounding box information
    \end{itemize}
    ...
    \item Ensembling
    \begin{itemize}
      \item Treat rows as independent, then aggregate predicted column segments
      \item Jointly train on all sequences in a table, mixing the individual rows
    \end{itemize}
  \end{itemize}
\end{frame}
